    ...
    <div>
        ...
        <div id="question-container" role="form" aria-labelledby="quiz-title">
            ...
            <div id="question" tabindex="0">
                <div id="error-message" :aria-live="errorMessage ? 'assertive' : 'off'"
                ...
                <div id="options-container" role="group" aria-labelledby="options-title" class="space-v-16">
                    ...
                    <template v-for="(option, index) in currentQuestion.opties" :key="index">
                        <label class="option-label space-8">
                            <input type="radio" v-model="selectedOption" :value="option" :name="'optie' + index"
                                :id="'optie' + index" :aria-labelledby="'optie' + index + '-label'" />
            ...
            <div>
                <Modal ref="resultModal" @close="restartQuiz" :title="`Uitslag Richtlijn ${quizId.replace('-', '.')}`">
                    ...
                    <div class="results_container" v-if="currentQuestionIndex === questions.length">
                        ...
                        <button class="space-h-8 secondary" @click="$router.push({ name: 'QuizOverview' })">Terug naar
                            overzicht</button>
                    </div>
                </Modal>
...
import { topicQuestions } from '../data/topicQuestions.js';
...
const quizId = computed(() => route.params.slug);

// const questions = questionsData[route.params.id];
// 10 random questions
// const questions = topicQuestions[route.params.id].sort(() => Math.random() - Math.random()).slice(0, 2);

// find questions by topic slug
const questions = topicQuestions.find((topic) => topic.slug === route.params.slug).vragen.sort(() => Math.random() - Math.random()).slice(0, 5);
const title = topics.find((topic) => topic.slug === route.params.slug).topic;
